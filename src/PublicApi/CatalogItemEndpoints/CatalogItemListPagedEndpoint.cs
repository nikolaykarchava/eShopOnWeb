...
public class CatalogItemListPagedEndpoint : IEndpoint<IResult, ListPagedCatalogItemRequest>
{
    ...
    private IAppLogger<ListPagedCatalogItemRequest> _logger;

    public CatalogItemListPagedEndpoint(IUriComposer uriComposer, IMapper mapper, IAppLogger<ListPagedCatalogItemRequest> logger)
    {
    ...
    public async Task<IResult> HandleAsync(ListPagedCatalogItemRequest request)
    {
        ...
        _logger.LogInformation($"CatalogItemListPagedEndpoint retrieved from db: {items.Count} items");
